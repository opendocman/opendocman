...
elseif(isset($_REQUEST['submit']) && $_REQUEST['submit'] == 'delete')
{
    ...
    echo '<center>'; 
    echo '<table border=0>';
    $query = 'SELECT table_name, display_name FROM udf where table_name="' . $_REQUEST['item'] . '"';
...
<?php
	$query = 'SELECT table_name,display_name FROM udf ORDER BY id';
	$result = mysql_query($query, $GLOBALS['connection']) or die ("Error in query: $query. " . mysql_error());
	while(list($lid, $lname) = mysql_fetch_row($result))
	{
...
elseif (isset($_REQUEST['submit']) && $_REQUEST['submit'] == 'edit')
{
    ...
    $query = 'SELECT table_name,field_type,display_name FROM udf WHERE table_name = "'.$_REQUEST['udf'].'"';
    ...
    if ( $field_type == 1 ) {
        ...
        if (isset($_REQUEST['display_name']) && $_REQUEST['display_name'] != "" ) {
            $query = 'UPDATE udf SET display_name="'.$_REQUEST['display_name'].'" WHERE table_name = "'.$_REQUEST['udf'].'"';
            mysql_query($query);
