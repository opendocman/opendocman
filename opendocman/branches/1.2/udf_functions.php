...
function udf_functions_add_udf()
{
  if ( $_REQUEST['field_type'] == 1 || $_REQUEST['field_type'] == 2) {
    $query = "INSERT {$GLOBALS['CONFIG']['db_prefix']}udf (id,table_name,display_name,field_type) VALUES (1,\"{$_REQUEST['table_name']}\",\"{$_REQUEST['display_name']}\",\"{$_REQUEST['field_type']}\")";
    mysql_query($query);
    $query = "ALTER TABLE {$GLOBALS['CONFIG']['db_prefix']}data ADD COLUMN '{$_REQUEST['table_name']}' int AFTER category";
    mysql_query($query);
