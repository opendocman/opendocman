...
$query = "INSERT INTO {$dbprefix}odmsys VALUES (NULL,'version','1.4.5')";
$stmt = $pdo->prepare($query);
$stmt->execute();
...
$query = "CREATE TABLE IF NOT EXISTS `{$dbprefix}email_queue` (
...
// Create email queue statistics table
echo 'Creating email queue statistics table...<br />';
$query = "CREATE TABLE IF NOT EXISTS `{$dbprefix}email_queue_stats` (
    `id` int(11) NOT NULL AUTO_INCREMENT,
    `date` date NOT NULL,
...
$email_queue_settings = array(
    "INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'email_queue_enabled', 'True', 'Enable asynchronous email queue processing. When enabled, emails are queued and processed in background.', 'bool');",
    "INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'email_queue_batch_size', '10', 'Number of emails to process in each queue batch. Lower values reduce server load.', 'num');",
    "INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'email_queue_retry_delay', '300', 'Delay in seconds before retrying failed emails (default: 5 minutes).', 'num');",
    "INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'email_queue_max_attempts', '3', 'Maximum number of retry attempts for failed emails.', 'num');",
    "INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'email_queue_cleanup_days', '30', 'Number of days to keep processed emails in queue before cleanup.', 'num');",
    "INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'email_queue_priority_high', '1', 'Priority level for high priority emails (lower numbers = higher priority).', 'num');",
    "INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'email_queue_priority_normal', '5', 'Priority level for normal priority emails.', 'num');",
    "INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'email_queue_priority_low', '10', 'Priority level for low priority emails.', 'num');"
);

foreach ($email_queue_settings as $query) {
    $stmt = $pdo->prepare($query);
    $stmt->execute();
}
