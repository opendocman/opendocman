...
if (
    ...
    $redirect = 'index?last_message=' . urlencode(msg('message_your_password_has_been_changed'));
    ...
    if ($stmt->rowCount() < 1) {
        $redirect = 'forgot_password?last_message=' . urlencode(msg('message_the_code_you_are_using'));
        header("Location: $redirect");
        exit;
    } else {
        $userInfo = $stmt->fetch();
    ...
    if ($stmt->rowCount() == 0) {
        $redirect = 'forgot_password?last_message=' . urlencode(msg('message_the_username_you_entered'));
        header("Location: $redirect");
        exit;
    } else {
        ...
        // generate the link
        $resetLink = $GLOBALS['CONFIG']['base_url'] . '/forgot_password?username=' . e::h($username) . '&code=' . e::h($reset_code);
        ...
        $redirect = 'forgot_password?last_message=' . urlencode(msg('message_an_email_has_been_sent'));
        header("Location: $redirect");
        exit;
    }
}
