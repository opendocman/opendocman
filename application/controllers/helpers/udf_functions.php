...
//namespace opendocman\controllers\helpers\udf_functions;
//class UDF_Functions { const load = 1; }

// (C) 2007  Stephen Lawrence Jr., Jonathan Miner
// adds user definced functions
...
if (!defined('udf_functions')) {
    ...
    function udf_functions_add_udf()
    {
        ...
        if (empty($_REQUEST['table_name'])) {
            header('Location: admin?last_message=' . urlencode(msg('message_udf_cannot_be_blank')));
            exit;
        }
        ...
        if ($stmt->rowCount() == 0) {
            if ($_REQUEST['field_type'] == 1 || $_REQUEST['field_type'] == 2) {
                ...
                if (!$stmt) {
                    ...
                    header('Location: admin?last_message=Error+:+Problem+With+Create');
                    exit;
                }
                ...
                if (!$stmt) {
                    ...
                    header('Location: admin?last_message=Error+:+Problem+With+INSERT');
                    exit;
                }
                ...
                ";
                    ...
                    header('Location: admin?last_message=Error+:+Duplicate+UDF+Name');
                    exit;
                }
            ...
            header('Location: admin?last_message=Error+:+Duplicate+Table+Name');
            exit;
        }
    }
