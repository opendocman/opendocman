...
echo 'Updating access_log table...<br />';
$query = "ALTER TABLE `{$_SESSION['db_prefix']}access_log` 
    ADD PRIMARY KEY (`file_id`, `user_id`, `timestamp`, `action`);";
// Added Primary Key
...
$query = "ALTER TABLE `{$_SESSION['db_prefix']}dept_perms` 
    CHANGE `fid` `fid` INT(11) UNSIGNED   NOT NULL FIRST, 
    CHANGE `dept_id` `dept_id` INT(11) UNSIGNED   NOT NULL AFTER `fid`, 
    CHANGE `rights` `rights` tinyint(1)   NOT NULL DEFAULT 0 AFTER `dept_id`, 
    DROP KEY `fid`, 
    ADD PRIMARY KEY (`fid`,`dept_id`);";
// Original Field format of 'dept_perms.fid' was NULL. No data modifications needed!
// Original Field format of 'dept_perms.dept_id' was NULL. No data modifications needed!
...
$query = "ALTER TABLE `{$_SESSION['db_prefix']}dept_reviewer` 
    CHANGE `dept_id` `dept_id` INT(11) UNSIGNED   NOT NULL FIRST, 
    CHANGE `user_id` `user_id` INT(11) UNSIGNED   NOT NULL AFTER `dept_id`, 
...
$query = "ALTER TABLE `{$_SESSION['db_prefix']}log` 
    CHANGE `revision` `revision` VARCHAR(255)   NOT NULL AFTER `note`, 
    DROP KEY `id`,
    ADD PRIMARY KEY (`id`, `revision`);";
// Original Field format of 'log.revision' was  VARCHAR(255) DEFAULT NULL. No data modifications needed!
// Added Primary Key
$stmt = $pdo->prepare($query);
$stmt->execute();
...
$query = "ALTER TABLE `{$_SESSION['db_prefix']}rights` 
	CHANGE `RightId` `RightId` TINYINT(1)   NOT NULL FIRST, 
	ADD PRIMARY KEY (`RightId`), 
	ADD UNIQUE KEY `UnqRight` (`Description`);";
...
$query = "ALTER TABLE `{$_SESSION['db_prefix']}user_perms` 
    CHANGE `fid` `fid` INT(11) UNSIGNED   NOT NULL FIRST, 
    CHANGE `uid` `uid` INT(11) UNSIGNED   NOT NULL AFTER `fid`,
...
// Original Field format of 'user_perms.uid' was INT(11) UNSIGNED DEFAULT NULL. No data modifications needed!
...
"INSERT IGNORE INTO `{$_SESSION['db_prefix']}filetypes` VALUES(NULL, 'video/3gpp', 1);",
